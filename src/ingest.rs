...
mod tests {
    ...
    #[test]
    fn event_time_without_session_dispatches_to_the_event_time_append() {
        // The four-way dispatch on (session_id, event_time) had test cover
        // for the store-layer branch functions but none for THIS match: a
        // rewire that sent `event_time: Some` down the plain append would
        // have passed every existing fixture. The sidecar read-back is the
        // observable difference.
        let mut store = Store::open_in_memory().unwrap();
        let mut req = request("fact with a declared fact-time");
        req.event_time = Some(EventTimeRange::point(injected_now()));
        let out = ingest(&mut store, req, defaults(), injected_now()).unwrap();
        let event = store
            .event_time_of(out.id.as_str())
            .unwrap()
            .expect("the declared fact-time reached the sidecar through the dispatch");
        assert_eq!(event.event_from(), injected_now());
        let stored = store.get(out.id.as_str()).unwrap().unwrap();
        assert_eq!(stored.session_id, None, "no session was declared");
    }

    #[test]
    fn event_time_with_session_dispatches_to_the_combined_append() {
        let mut store = Store::open_in_memory().unwrap();
        store.open_session("sess-ingest", injected_now()).unwrap();
        let mut req = request("fact with a session and a declared fact-time");
        req.session_id = Some("sess-ingest".to_string());
        req.event_time = Some(EventTimeRange::point(injected_now()));
        let out = ingest(&mut store, req, defaults(), injected_now()).unwrap();
        let stored = store.get(out.id.as_str()).unwrap().unwrap();
        assert_eq!(
            ...
            Some("sess-ingest"),
            "the session link landed"
        );
        let event = store
            .event_time_of(out.id.as_str())
            .unwrap()
            .expect("the declared fact-time landed beside the session link");
        assert_eq!(event.event_from(), injected_now());
    }
