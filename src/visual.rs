...
/// Contract-order rank of a relation kind — delegates to the enum's own
/// contract order ([`RelationKind::rank`], derived from `ALL`); three local
/// copies of this ordering drifted in review once, so none survives.
fn kind_rank(kind: RelationKind) -> usize {
    kind.rank()
}
