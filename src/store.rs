...
impl RelationKind {
    ...
    /// which already declares the contract order. Derived, never a second
    /// table — review found three hand-written copies of this ordering, and
    /// a copy that cannot exist cannot drift. A test on the relation module
    /// pins this order to `relation::RelationKind::ALL` wire name by wire
    /// name, so the two vocabularies cannot diverge silently.
    #[must_use]
...
impl Store {
    ...
    /// Snapshot a store FILE without ever opening it as a `Store`. The
    /// source connection is READ-ONLY and no schema code runs, so this can
    /// never migrate, repair, or create anything at `source_path`.
    ///
    /// This exists for `nmemory backup`, whose whole contract is "copy the
    /// pre-upgrade state BEFORE the one-way door". Routing backup through
    /// [`Store::open`] destroyed exactly that state twice over: `open`
    /// CREATES a missing file (a typo'd `--db` yielded a successful backup
    /// of a brand-new empty store) and MIGRATES an old schema in place (the
    /// pre-migration bytes the copy was for were gone before the copy
    /// began). A read-only open of a missing file fails instead of
    /// creating, and a source with an unknown FUTURE schema still copies —
    /// a backup tool that refuses the file you most need preserved is no
    /// backup tool.
    pub fn snapshot_file(source_path: &Path, destination_path: &Path) -> Result<(), StoreError> {
        let conn = Connection::open_with_flags(source_path, OpenFlags::SQLITE_OPEN_READ_ONLY)
            .map_err(backend)?;
        conn.backup(MAIN_DB, destination_path, None)
            .map_err(backend)
    }
